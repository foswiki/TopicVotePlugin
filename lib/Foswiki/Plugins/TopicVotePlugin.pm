...
sub initPlugin {
    ...
    $topicVoteAdmin =
      Foswiki::Func::getPluginPreferencesValue("TOPICVOTE_ADMIN");
...
sub _topicvotesetup {
    ...
    my ( $logMeta, $logTopicdata ) =
      Foswiki::Func::readTopic( $webName, $config{TOPICLOG} );
    ...
    Foswiki::Func::saveTopic( $webName, $config{TOPICLOG}, $logMeta,
        $logTopicdata );
...
sub _topicvote {
    ...
    $config{TOPICLOG} =
      $Foswiki::Plugins::SESSION->normalizeWebTopicName( $config{CONFWEB},
        $config{CONFTOPIC} . "Log" );
...
"\n| <form action=\"%SCRIPTURLPATH{\"rest/TopicVotePlugin/savevote\"}%\" method=\"post\"><input type=\"text\" size=\"3\" id=\"credits\" name=\"credits\"$disable_form />";
    $voting_form .=
" <input type=\"submit\" value=\"vote!\" class=\"foswikiButton\"$disable_button />";
    $voting_form .=
    ...
    $voting_form .=
      ...
      . "\" name=\"conf_t\" />";
    $voting_form .=
      " <input type=\"hidden\" value=\"" . $max_points . "\" name=\"max_p\" />";
    $voting_form .= "</form> |\n";
...
sub _createPollLog {
    ...
    # get topic text of configuration topic
    my $topicdata =
      Foswiki::Func::readTopic( $config{CONFWEB}, $config{TOPICLOG} );
    ...
    $topicdata =~
s/^\|\s*(.*?)\s*\|\s*($webname_p\.)?($wikiword_p)\s*\|\s*($webname_p\..*?)\s*\|\s*(\d+)\s*\|.*$/_logmap($1,$3,$4,$5)/mego;
...
m/^(\s*\|\s*\*date\*\s*\|\s*\*username\*\s*\|\s*\*topic\*\s*\|\s*\*credit points\*\s*\|)\s*$/mgo
    ...
    if ( $topicdata =~ s/^$theader/$log/mgo ) {
        my $user_backup = $session->{user};
        my $votingAdminUserID =
          Foswiki::Func::getCanonicalUserID($topicVoteAdmin);
        if (
            Foswiki::Func::checkAccessPermission(
                "CHANGE",          $votingAdminUserID, $topicdata,
                $config{TOPICLOG}, $config{CONFWEB},   $meta
            )
          )
        {
            $session->{user} = $votingAdminUserID;
            Foswiki::Func::saveTopic( $config{CONFWEB}, $config{TOPICLOG},
                $meta, $topicdata );
        }
        else {
            $session->{user} = $user_backup;
...
m/^\|\s*($webname_p\.)?$config{USER}\s*\|\s*(\d+)\s*\|\s*.*?\s*\|\s*(.*?)\s*\|$/mgo
          )
        {
            ...
            if ( $topicdata =~ s/$confentry/$editentry/mgo ) {
                ...
                if (
                    Foswiki::Func::checkAccessPermission(
                        "CHANGE",         $votingAdminUserID,
                        $topicdata,       $config{CONFTOPIC},
                        $config{CONFWEB}, $meta
                    )
                  )
                {
                    $session->{user} = $votingAdminUserID;
                    Foswiki::Func::saveTopic( $config{CONFWEB},
                        $config{CONFTOPIC}, $meta, $topicdata );
                }
                else {
                    $session->{user} = $user_backup;
                    Foswiki::Func::saveTopic( $config{CONFWEB},
...
sub _restSaveVote {
    ...
    if (
        ( $max_points > 0 )
        && ( ( _getSharedPoints($voted_topic) + $submitted_credits ) >
            $max_points )
      )
    {
...
sub _getSharedPoints {
    ...
    while ( ( my $date, my $points ) =
        each( %{ $pollLog{ $config{USER} }{$voted_topic} } ) )
    {
        $sharedPoints += $points;
    }
...
#return $Foswiki::Plugins::SESSION->redirect( Foswiki::Func::getViewUrl($voted_web, $voted_topic), 0 );
    return Foswiki::Func::redirectCgiQuery( undef,
        Foswiki::Func::getViewUrl( $voted_web, $voted_topic ), 0 );
}
